...
func _physics_process(delta: float):
	...
	# If disabled then turn off physics processing and bail out
	if !enabled:
...
func move_player(prev_velocity: Vector3) -> Vector3:
	...
	if push_rigid_bodies:
		for idx in range(get_slide_collision_count()):
			...
			if obj.is_class("RigidBody3D"):
				...
				var impact_velocity = prev_velocity - rb.linear_velocity
